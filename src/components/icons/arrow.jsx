...
const ArrowIcon = ({ size, color, direction = 'right' }) => {
  const svgClasses = `w-${size} h-${size}`;
  const fillColor = color || '#475569'; // Default color

  // Function to calculate rotation based on direction
  const getRotationDegree = (direction) => {
    switch (direction) {
      case 'up':
        return 'rotate(-90 8 8)'; // Rotates 90 degrees counter-clockwise
      case 'down':
        return 'rotate(90 8 8)';  // Rotates 90 degrees clockwise
      case 'left':
        return 'rotate(180 8 8)'; // Rotates 180 degrees
      case 'right':
      default:
